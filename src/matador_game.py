...
# Player piece
player_tile = 0  # index of the current tile (0 to n_brikker-1)
...
def handle_events():
    ...
    global player_tile, popup_open, popup_tile
        ...
        if event.type == pygame.QUIT:
            ...
            if not popup_open:
                if event.key == pygame.K_a:
                    player_tile = (player_tile + 1) % n_brikker
...
def draw_player():
    pos = get_tile_world_pos(player_tile)
    screen_pos = zoomed(pos)
    radius = int(30 * zoom)
    pygame.draw.circle(screen, player_color, (int(screen_pos.x), int(screen_pos.y)), radius)
    pygame.draw.circle(screen, BLACK, (int(screen_pos.x), int(screen_pos.y)), radius, max(1, int(2 * zoom)))
